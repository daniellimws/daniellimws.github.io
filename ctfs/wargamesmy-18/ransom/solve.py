...
def main():
    browser = 'chrome'
    dubs = [0.742293984219203562, 0.593954303952432650, 0.779133218474818312]
    state0, state1 = 12614987376877701029, 4208293804791191969
    dubs.append(to_double(browser, (state0 + state1) & 0xFFFFFFFFFFFFFFFF))
    # state0, state1 = 15033139301404491003, 12614987376877701029

    locked = open("flag.zip.locked").read()
    for i in range(32, len(locked), 8):
        state0, state1, out = xs128p_backward(state0, state1)
        out = to_double(browser, out)
        dubs.append(out)
    ...
    for i in range(0, len(locked), 8):
        rec += xor(locked[i:i+8], keys[i // 8].decode('hex'))

    open("flag.zip", "w").write(rec)
